...
def get_cells_loaders(args):
    trainset = CellsDataset(args.data_root, mode = 'train')
    trainloader = DataLoader(trainset, batch_size=args.batch_size, shuffle=True,
                             pin_memory=True, num_workers=4, drop_last=True)
    
    testset = CellsDataset(args.data_root, mode = 'val')
...
def main():
    ...
    trainloader, testloader = get_cells_loaders(args)
